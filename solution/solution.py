...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        history = history[-6:]   # giữ tối đa 3 lượt gần nhất
...
def run_assistant(
    ...
    while True:
        ...
        stream = retry_with_backoff(
            lambda messages=messages: client.chat.completions.create(
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    columns = ("Prompt", "GPT-4o Response", "Mini Response",
               "GPT-4o Latency", "Mini Latency")
    header = " | ".join(f"{col:<40}" if col not in ("GPT-4o Latency", "Mini Latency")
                         else f"{col:<15}" for col in columns)
    lines = [header, "-" * len(header)]

    for r in results:
        row = " | ".join([
            f"{_truncate(r['prompt']):<40}",
            f"{_truncate(r['gpt4o_response']):<40}",
            f"{_truncate(r['mini_response']):<40}",
